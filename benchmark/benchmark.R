...
blogFiles <- file.path(benchmarkFolder, "blogs.txt")

tweets <- readLines(twitterFiles, encoding = 'UTF-8')
...
blogs <- readLines(blogFiles, encoding = 'UTF-8')
...
predict.baseline <- function(x){
  ...
  bmSentTokens <- tokenizeSentences(x)
  bmNgram1 <- tokenizeNgrams(bmSentTokens, 1)
  
  #system.time(bmSentTokens <- tokenizeSentences(x))
