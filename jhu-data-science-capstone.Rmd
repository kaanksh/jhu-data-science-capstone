...
## Introduction
This is the Milestone Report for Capstone project of the [Johns Hopkins Data Science Specialization on Coursera](https://www.coursera.org/specializations/jhu-data-science). The goal of this project is to build a model and a Shiny web app to predict the next word a user will type. This is similar to what is done by most smartphone keyboards, like Swiftkey, the project's partner.

The data to use initially comes from a corpus called [HC Corpora](www.corpora.heliohost.org). They contain text from news articles, blogs and twitter. We will use only the training data that has beed provided on Coursera by the JHU Data Science team.

This report aims to showcase the initial exploratory data analysis. It will also present briefly the next steps that will be taken to build the prediction model and to develop the Shiny wep app.
...
Since the data is fairly large, only a random `r I(SAMPLE_RATE * 100)`% sample will be used for this initial report. This is to reduce the time needed for the pre-processing and the creation of the document-term matrixes.
...
* Remove hastags ("#Coursera")

* Remove all punctuation

* Remove all numbers

* Remove a list of pre-defined swear words
...
Note : this type of pre-processing will introduce gaps in the text. Depending on the context, this might create incoherrent n-grams :

* Example 1 : "This is \<swear_word\> stupid." becomes "This is stupid". This still produces coherent n-grams.

* Example 2 : "This is \<swear_word\>. I'm leaving." becomes "This is I'm leaving". Both the swear word and the full stop are removed which can create some incoherrent n-grams.
...
Attention will be paid to the trade-off between acccuracy and efficiency (prediction time and RAM used). Indeed, for a smartphone keyboard application, it is pointless to gain 5% accuracy at the cost of a 5 seconds prediction time. 
